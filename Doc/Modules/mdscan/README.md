...
> замеры [H-04](../../../MemoryBank/specs/hw01_h04_baseline_2026-08-17.md) →
> [H-07](../../../MemoryBank/specs/hw01_h07_after_2026-08-17.md),
> [нагрузка](../../../MemoryBank/specs/hw01_h09_load_2026-08-17.md),
> [устойчивость](../../../MemoryBank/specs/hw01_h08_resilience_2026-08-17.md),
> ревью [спеки](../../../MemoryBank/specs/hw01_h10_review_spec_2026-08-17.md) и
> [кода](../../../MemoryBank/specs/hw01_h11_review_code_2026-08-17.md).
...
(`0` чисто · `1` есть битые · `2` ошибка аргументов · `3` внутренняя ошибка · `130` прервано
пользователем).
...
│                                resolved_path_cache            один resolve() на файл (этап 2)
├── parsing/                     markdown_reader · link_extractor · markdown_it_link_extractor
│   │                            markdown_it_heading_source · link_classifier
│   └── rules/                   link_rule + 10 правил классификации
│                                (10-е — rule_other_scheme: data:, javascript:, ftp:, std:: → UNKNOWN)
...
```text
core/tokenstat/
├── __main__.py                  CLI: python -m core.tokenstat --since … --label … --out …
...
Инструменты замера (не тесты — `pytest` собирает только `test_*.py`) лежат в
[`tests/hw01/support/`](../../../tests/hw01/support): `bench_scan.py` (полный прогон, слои,
cProfile), `bench_layers.py` (разложение по слоям), `bench_load.py` (нагрузка 500/2000/5000 файлов,
`tracemalloc`, backlog очередей), `bench_http.py` + `bench_http_server.py` (`speedup` с HTTP на
локальном сервере с управляемой задержкой).
...
| Chain of Responsibility | `ValidationChain` (V1…V10) · `LinkClassifier` (10 правил) |
...
Все тесты лежат в [`tests/hw01/`](../../../tests/hw01) — 17 файлов, **444 теста** (плюс 4 `skipped`:
живые HTTP-проверки идут только при `MDSCAN_NETWORK=1`).
...
Все числа — набор B: 500 сгенерированных файлов, 1025 ссылок; HTTP, лог, прогресс и консольная
сводка выключены; медиана из 5 прогонов после одного прогрева; Windows 11, Python 3.14.
Инструмент лежит в репозитории и повторяет замер одной командой:
...
python tests/hw01/support/bench_load.py   --sizes 500,2000,5000 --repeat 5 --leak 5000
```

### 12.1 Слои прогона: до и после оптимизаций

| Слой (последовательно, без потоков) | До | После | Чем вызвано изменение |
|---|---:|---:|---|
| Стадия 1: обход (`scandir` + `resolve` + реестр) | 0.121 с | **0.060 с** | `os.scandir` вместо `rglob`, один `resolve()` на файл (`ResolvedPathCache`), реестр не резолвит абсолютные пути повторно |
| Чтение 500 файлов | 0.025 с | 0.025 с | не трогали |
| Извлечение ссылок (`markdown-it`, полный разбор) | 0.088 с | 0.088 с | не трогали — это единственный полный разбор файла |
| Классификация (10 правил) | 0.003 с | 0.003 с | — |
| Проверка локальных и якорей, холодный кэш | 0.186 с | **0.082 с** | блочный разбор заголовков + кэш базового каталога в `LocalFileChecker` |
| Проверка, тёплый кэш заголовков | 0.078 с | **0.015 с** | повторные `resolve()`/`exists()` ушли |
| → из них разбор ради заголовков | 0.108 с | **0.067 с** | блочный разбор сам стоит 0.043 с, остальное — чтение файла внутри `AnchorChecker` |
| «Полезная нагрузка» последовательно | 0.319 с | **0.225 с** | сумма выше |
| Накладные (пул, две очереди, `join`, логгер, отчёт) | 0.166 с | **0.109 с** | ленивое логирование + меньше работы под очередью |
| **Полный прогон, 1 поток** | **0.605 с** | **0.389 с** | −36 % |
| Вызовов `nt._getfinalpathname` за прогон | 4 395 | **536** | −88 % |

Баланс сходится: `0.389 ≈ 0.060 (обход) + 0.225 (нагрузка) + 0.109 (накладные)`.

### 12.2 Где узкие места сейчас

| # | Статья | Время | Доля | Что с этим можно сделать |
|---|---|---:|---:|---|
| 1 | Накладные конвейера (пул потоков, две очереди, старт/стоп логгера, сборка отчёта) | 0.109 с | 28 % | батч задач (N файлов на задачу) — трогает правило владения и порядок гашения, только по согласованию |
| 2 | Извлечение ссылок: полный разбор `markdown-it` | 0.088 с | 23 % | отключить лишние блочные правила (`md.disable`) — риск потерять синтаксис, только с тестом эталонного набора |
| 3 | Проверка локальных и якорей (0.043 с из них — блочный разбор заголовков) | 0.082 с | 21 % | дальше — только не читать файл дважды, а это нарушит разделение владения `HeadingSource` |
| 4 | Обход (`scandir` + `resolve`) | 0.060 с | 15 % | упирается в ОС |
| 5 | Чтение файлов + классификация | 0.028 с | 7 % | упирается в диск |

Самой крупной одиночной статьи расходов больше нет: после блочного разбора заголовков они размазаны
ровно, и дальнейшая экономия без изменения архитектуры — единицы процентов.

### 12.3 Границы применимости параллельности

| Режим | 1 поток | 5 потоков | 10 потоков | `speedup` |
|---|---:|---:|---:|---:|
| **Без HTTP** (500 файлов) | 0.389 с | 0.403 с | 0.432 с | 0.96 / **0.90** |
| **С HTTP**, локальный сервер 30 мс, 600 уникальных URL | 25.07 с | 4.87 с | 2.61 с | ×5.15 / **×9.59** |
...
1. **Без сети потоки не ускоряют** — работа CPU-bound под GIL, файловые вызовы короткие и
   кэшируются ОС. 10 потоков стоят ≈10 % времени; умолчание `workers.parse = 10` оправдано только
   тем, что HTTP включён по умолчанию. `ProcessPoolExecutor` не применяется: он ломает правило
   владения объектом, а старт процессов на Windows съест выигрыш на задаче в полсекунды.
2. **С сетью масштабирование почти линейное** до `min(workers.parse, http.workers)` — пик
   одновременных запросов на сервере ровно 1 / 5 / 10. Поднимать `workers.parse` выше
   `http.workers` бессмысленно: лишние потоки встанут в семафор.
3. **Единица параллельной работы — файл, а не ссылка.** Один `.md` — одна задача одного потока,
   его ссылки проверяются последовательно. Прогон по одному большому файлу не ускорится ни при
   каком числе потоков.
4. **Потолок живого прогона задаёт самая медленная ссылка.** Отказ DNS занимает 10–12 с, и
   `http.timeout_ms` его не покрывает (`socket.getaddrinfo` таймаутом `urlopen` не ограничен).

### 12.4 Масштаб и стоимость лога

| Файлов | Ссылок | Время (1 поток) | Файлов/с | Пиковая память | Пик `task_qsize` | Отчёт |
|---:|---:|---:|---:|---:|---:|---:|
| 500 | 1025 | 0.390 с | 1283 | 3.9 МиБ | 440 | 75 КиБ |
| 2000 | 3971 | 1.360 с | 1471 | 10.8 МиБ | 1884 | 307 КиБ |
| 5000 | 10087 | 3.255 с | 1536 | 24.6 МиБ | 4879 | 785 КиБ |

- Время растёт **строго линейно** и даже чуть лучше: `0.078 с + 0.636 мс × N` (r² = 0.99997),
  цена одного файла падает с 0.78 до 0.65 мс за счёт амортизации постоянной части (≈0.08 с).
  Экстраполяция: 50 000 файлов ≈ 32 с, 100 000 ≈ 64 с.
- **Память не течёт**: три прогона подряд в одном процессе дают один и тот же пик 28.0 МиБ и
  остаток 0.1 МиБ — `PipelineRunner` создаётся заново на каждый `scan()`, `_shutdown` обнуляет
  ссылку на него, и вместе с ним умирают очереди, результаты и кэши чекеров.
- **Границы**: очереди не имеют `maxsize` (завершение идёт по сентинелам, обратного давления в
  конвейере нет по проекту), поэтому backlog доходит до 98 % дерева, а пиковая память — ≈4.6 КиБ
  на файл. 100 000 файлов → ≈0.5 ГБ. Размер отчёта тоже линеен, ≈158 байт на файл → ≈15 МиБ
  Markdown на 100 000 файлов. Это ограничения, а не поломки.
- **Цена лога**: на 5000 файлов `logging.level: INFO` даёт 7 717 строк и +14 % времени, а `DEBUG` —
  67 739 строк, 8.6 МиБ и **×2** ко времени прогона. Умолчание `INFO` выбрано именно поэтому.
- **Стоимость прогона определяется числом ссылок, а не байтами**: файл на 10 МБ с 1066 ссылками
  разбирается 2.25 с, тот же объём с 64 536 ссылками — 20 с.
- `duration_sec` в отчёте не включает отрисовку консольной сводки (на 634 битых ссылки `rich`
  рисует 7 с) и удаление клонов — это фазы после замера.

### 12.5 Клонирование организации

Клоны идут собственным пулом на `workers.discover` потоков (`GitHubOrgSource`), `RepoInfo` отдаются
по мере готовности. Замер на организации `dsp-gpu` (10 репозиториев, HTTP выключен):
последовательно **45.2 с** → параллельно **31.5 с**. До этого клонирование занимало 72.5 с из 103 с
полного боевого прогона — 70 % времени, и `workers.discover` на организацию не действовал вовсе:
пул стадии 1 параллелит по **источникам**, а организация — один источник.

---

## 13. Что нового узнали по ходу разработки
...
- **GIL и «параллельность».** Без HTTP разбор Markdown — чисто процессорная работа, потоки её
  не ускоряют: измеренный `speedup ≈ 0.90…0.96`. Ожидание 1.5–3× относится к прогону с сетевыми
  проверками, и там оно перекрыто с запасом (×9.59 на 10 потоках). Честное число записано
  в метрики ДЗ, а не подогнано под спеку.
...
### Что добавила боевая приёмка (этап 2)

- **`cProfile` при потоках врёт по `cumtime`.** Профиль показывал `nt._getfinalpathname` с
  `cumtime` **больше 100 %** общего времени (1.30 с при прогоне 1.26 с, а на 10 потоках — 206 %):
  рекурсивная свёртка `ncalls 4395/2249` складывает время вызовов из разных потоков повторно.
  Честная величина — `tottime` = 0.16 с ≈ 13 %. Вывод «узкое место — `resolve()`» был ложным, и
  оптимизация по нему лечила бы не то. **Смотреть `tottime`, а слои мерить `perf_counter` вручную.**
- **90 % вызовов `logging.debug` — чужие.** Из 49 585 вызовов за прогон mdscan делает 4 738, а
  44 746 приходятся на сам `markdown-it`: его блочные правила зовут `LOGGER.debug` на каждую попытку
  правила, независимо от уровня. Убрать можно было только свою долю.
- **`data:`-URI нельзя отдавать «тотальному» правилу пути.** `LocalPathRule` — последнее правило
  цепочки, оно ловит всё, что не подошло раньше, поэтому встроенная картинка
  `![](data:image/png;base64,…)` считалась локальным путём, признавалась битой и целиком уезжала
  в отчёт: **одна строка на 1.8 МБ**, 89 % объёма файла. Лечится двумя независимыми правками:
  правило чужих схем перед `LocalPathRule` и обрезка ячейки отчёта до 200 символов. Побочный
  выигрыш — C++-символы вида `std::string` из автодоков перестали быть «битыми ссылками».
- **`linkify` в fuzzy-режиме превращает имена файлов в URL.** `Full.md`, `conftest.py`, `prng.cl`
  становились внешними ссылками и давали 51 ложный `BROKEN` на боевом прогоне. Голые URL
  распознаются **только со схемой** (`fuzzy_link=False`).
- **`403` от живого сайта — это не битая ссылка.** Научные издательства и порталы патентов
  (researchgate, wiley, mdpi, sciencedirect, mathworks, uspto) отбивают запрос без браузерного
  `User-Agent`. Такие исходы вынесены в отдельную секцию отчёта «вероятно защита от ботов или лимит»:
  в статистике они остаются `BROKEN`, но человеку сразу видно, что чинить нечего.
- **`http.timeout_ms` — это таймаут одной сетевой операции, а не бюджет запроса.** Разрешение имени
  (`socket.getaddrinfo`) им не покрывается: несуществующий домен ждёт 10–12 с независимо от
  настройки. Цепочка редиректов получает таймаут заново на каждое соединение, поэтому адрес
  с `301` может уложиться в 100 мс, а прямой — нет.
- **`KeyboardInterrupt` — не `Exception`.** `except Exception` его не ловит, поэтому `Ctrl+C`
  выпускал трейсбек, закрывал лог раньше воркеров (их строки уходили на stderr «сырыми») и
  оставлял 11 не-демонов дорабатывать всю очередь — процесс жил минутами. Гашение потребовало
  отбрасывать очередь, **возвращая сентинел на место**, иначе воркер навсегда остаётся на `get()`.
- **`Ctrl+C` дочернему процессу на Windows из скрипта не доставить**: ни `kill -INT` из Git-Bash,
  ни `CTRL_C_EVENT` через `GenerateConsoleCtrlEvent`, ни `interrupt_main()` (он не будит
  заблокированное `queue.join()`). Прерывание пришлось воспроизводить в той же точке кода,
  куда его приводит живой `Ctrl+C`.
- **`Thread` в Python 3.14 занял ещё имена.** Помимо `_context` наследнику нельзя брать `_args`,
  `_target`, `_name` — ловушка, которой не было на 3.11/3.12.
- **Живая сеть недетерминирована**: один и тот же домен между двумя прогонами дал `403` и
  `TIMEOUT`. Для регрессии годится только локальный сервер с управляемой задержкой; живая сеть —
  справочный второй замер.
- **`tracemalloc` стоит ×2.3 по времени**, поэтому память и время меряются разными прогонами.

---

## 14. Стоимость разработки в токенах

Считает сам проект — `core/tokenstat/` по JSONL-транскрипту сессии и файлам субагентов.

| Этап | Агентов | requests | out | cache_create | cache_read | thinking | всего токенов |
|---|---:|---:|---:|---:|---:|---:|---:|
| 1 — разработка (T-01…T-16) | 17 запусков | 627 | 762 606 | 2 750 132 | 91 689 622 | 272 664 | **95 203 656** |
| 2 — приёмка и ускорение (H-01…H-13, включая H-12) | 14 запусков | 737 | 639 998 | 2 747 828 | 125 553 051 | 209 529 | **128 947 191** |
| **Итого** | **31 запуск** | **1364** | **1 402 604** | **5 497 960** | **217 242 673** | **482 193** | **≈224.2 млн** |

Разбивка по таскам и по агентам — в
[README ДЗ hw01](../../../homework/hw01_mdlinks/README.md) и в отчётах `out/hw01/tokens_*.md`.
Порядок расходов задаёт **не** генерация текста (1.3 млн output на оба этапа), а перечитывание
контекста: 195 млн `cache_read` — это спека, правила проекта и чужой код, которые каждый агент
поднимает заново.

---

*Документация модуля `core.mdscan`, 2026-08-17 (этап 2). Реальные числа прогона —
в [README ДЗ hw01](../../../homework/hw01_mdlinks/README.md). Контракты классов на этапе 2
не менялись.*
