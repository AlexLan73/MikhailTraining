...
workers:                           # ПАРАЛЛЕЛЬНОСТЬ. Два пула потоков (третье число — http.workers,
  ...
  parse: 10                        # сколько .md-ФАЙЛОВ разбирается одновременно (🔧 р6, Alex: было 5;
                                   #   с HTTP 10 потоков → ×10, без HTTP +8 % — разбор под GIL не масштабируется).
...
http:                              # ПРОВЕРКА внешних ссылок
  ...
  workers: 10                      # СЕМАФОР: сколько HTTP-запросов идёт одновременно (🔧 р6: было 5)
